...
#######################################
my $output_CSV="D:\\cmo_disclaimer.csv";                    ##Output
my @languages=('en_US','it_IT','zh_CN_cor');      ## Global Variables...
my $dcr_path = "D:\\Test_DCR\\cmo\\en_US";             ##base directory
my @files_dcr;
my ($headline_copy_en_US,$headline_copy_it_IT,$headline_copy_es_CA,$headline_copy_fr_FR,$headline_copy_pt_BR,$headline_copy_zh_CN_cor);
...
foreach my $targetFile (@files)
{
  ...
  #  $csv->print ($fh,[$file_name,$headline_copy_en_US,$headline_copy_pt_BR,$headline_copy_zh_CN_cor]) ;
  print $headline_copy_en_US;
...
sub get_value{
	...
	my $xp = XML::XPath->new(parser => $p, filename => $targetFile);
  	foreach my $xmlrepoNode ($xp->find('/record/item[@name="disclaimer_domestic"]/value/text()')->get_nodelist)
